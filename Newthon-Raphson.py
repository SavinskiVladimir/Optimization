...
class Optimizer:
    ...
    def optimize(self):
        ...
        while k < self.max_iter:
            ...
            # проверка положительной определённости гессианны
            if np.all(np.linalg.eigvals(H) > 0):
                pass
            else:
                H_inv = np.eye(N=H.shape[0], M=H.shape[1], dtype=H.dtype)

            print(f"Матрица Гёссе:\n{H}"
                  f"\nОбратная матрица Гёссе:\n{H_inv}")

            p_k = -grad_vec
            h_k = np.dot(grad_vec, p_k) / np.dot(H @ p_k, p_k)

            print(f"Новый шаг: {h_k}")

            self.x -= h_k * np.dot(H_inv, p_k)

            print(f"Координаты нового вектора: {round(self.x[0], 3)} {round(self.x[1], 3)}"
